...
return <><section className="home-contact-section"><div className="container-page home-contact-grid"><div><p className="home-kicker">KẾT NỐI LADYSTARS</p><h2>Chọn cách bắt đầu phù hợp với bạn.</h2><p>Đội ngũ LADYSTARS sẵn sàng lắng nghe nhu cầu và giúp bạn tìm đúng bước tiếp theo.</p></div><div className="home-contact-cards"><Link to="/lien-he"><CalendarDays size={23} /><div><h3>Đặt lịch tư vấn</h3><p>Chọn thời điểm thuận tiện để bắt đầu cuộc trò chuyện.</p></div><ArrowRight size={19} /></Link><Link to="/dich-vu-cham-soc"><MessageCircle size={23} /><div><h3>Nhận hướng dẫn</h3><p>Tìm hiểu các tiêu chí quan trọng trước khi lựa chọn.</p></div><ArrowRight size={19} /></Link></div></div></section><section className="container-page home-section" aria-labelledby="insights-title"><div className="home-section-heading home-section-heading-center"><p className="home-kicker">CẨM NANG LADYSTARS</p><h2 id="insights-title">Thông tin hữu ích trước khi lựa chọn</h2></div><div className="home-insight-grid">{insightLinks.map(([title, description, path], index) => <Link to={path} key={title} className={index === 0 ? 'is-featured' : ''}><span>0{index + 1}</span><h3>{title}</h3><p>{description}</p><strong>Đọc thêm <ArrowRight size={17} /></strong></Link>)}</div></section></>
}
...
return <div className={`home-contact-dock ${open ? 'is-open' : ''}`}><div className="home-contact-dock-links"><Link to="/lien-he" aria-label="Đặt lịch tư vấn"><CalendarDays size={19} /></Link><Link to="/dich-vu-cham-soc" aria-label="Xem hướng dẫn lựa chọn"><MessageCircle size={19} /></Link><button type="button" aria-label="Trở về đầu trang" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>↑</button></div><button type="button" className="home-contact-dock-trigger" aria-label="Mở hỗ trợ" aria-expanded={open} onClick={() => setOpen((current) => !current)}><MessageCircle size={20} /><span>Hỗ trợ</span></button></div>
}
