...
export function NewsImage({ article, className = '', priority = false }: { article: Pick<NewsArticleSummary, 'cover_image_path' | 'cover_image_alt' | 'title'>; className?: string; priority?: boolean }) {
  if (!article.cover_image_path) return <span className={`news-image-placeholder ${className}`} role="img" aria-label={`Ảnh minh họa cho ${article.title}`}><span>LADYSTARS</span><strong>LS</strong></span>
