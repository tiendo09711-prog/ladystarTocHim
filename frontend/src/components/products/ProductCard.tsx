...
export function ProductCard({ product, variant = 'default' }: { product: Product; variant?: 'default' | 'catalog' }) {
  ...
  return <article className={`card product-card group overflow-hidden ${variant === 'catalog' ? 'flex h-full flex-col' : ''}`}>
    <Link to={`/san-pham/${product.slug}`} className="product-card-image block aspect-square overflow-hidden"><img src={image?.image_path || '/images/product-placeholder.svg'} alt={image?.alt_text || product.name} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.03]" /></Link>
