  ...
  return <div className="store-shell">
    ...
    <header className={`store-header ${isScrolled ? 'is-scrolled' : ''}`}>
      <div className="container-page store-header-main">
        ...
        <div className="store-header-actions"><button type="button" className="store-icon-button store-desktop-search" onClick={() => setSearchOpen((current) => !current)} aria-label="Mở tìm kiếm" aria-expanded={searchOpen}><Search size={21} /></button><Link to={accountPath} className="store-icon-button" aria-label={user ? 'Tài khoản của bạn' : 'Đăng nhập'}><UserRound size={21} /></Link><Link to="/gio-hang" className="store-icon-button store-cart-button" aria-label={`Giỏ hàng có ${count} sản phẩm`}><ShoppingBag size={21} />{count > 0 && <span>{count}</span>}</Link><button type="button" className="store-booking-link" onClick={() => setBookingOpen(true)}><CalendarDays size={18} /><span>Đặt lịch tư vấn</span></button></div>
      </div>
