...
export interface Service { id: number; name: string; slug: string; short_description?: string | null; price: number; image_path?: string | null; image_alt?: string | null; sort_order: number; status: 'active' | 'inactive' }
...
export interface OrderItem { id: number; product_name: string; variant_description?: string; variant_snapshot?: VariantSnapshotItem[] | null; sku: string; unit_price: number; quantity: number; line_total: number; review?: CustomerReview | null }
...
export interface Payment { method: string; provider: string; amount: number; status: 'pending' | 'paid' | 'refunded'; transaction_code?: string | null; verified_by?: number | null; paid_at?: string | null; note?: string | null }
