...
export interface Category { id: number; name: string; slug: string; description?: string; image_path?: string; is_active: boolean; show_in_menu?: boolean; sort_order?: number; children?: Category[] }
...
export interface HairGuideContent extends CatalogContent { services: Service[]; contact: { support_phone?: string | null; support_email?: string | null; appointments_enabled: boolean } }
...
export interface PaymentMethods { configured?: boolean; shipping?: { fee: number; free_from: number }; cod: { enabled: boolean }; bank_transfer: { enabled: boolean; bank_name?: string | null; account_name?: string | null; account_number?: string | null; bank_branch?: string | null; qr_path?: string | null; instruction?: string | null } }
export interface Order { id: number; order_number: string; total_amount: number; subtotal: number; discount_amount: number; shipping_fee: number; payment_method: string; payment_status: string; order_status: string; can_buy_again?: boolean; created_at: string; customer_name: string; customer_phone: string; province?: string; district?: string; ward?: string; shipping_address: string; admin_note?: string | null; items: OrderItem[]; status_histories?: OrderStatusHistory[]; payment?: Payment | null; shipment?: Shipment | null }
...
export interface AfterSalesShipment { id: number; purpose: string; carrier?: string | null; tracking_number?: string | null; status: 'pending' | 'shipped' | 'delivered' | 'delivery_failed' | 'returned'; shipped_at?: string | null; delivered_at?: string | null; failed_at?: string | null; failure_reason?: string | null; returned_at?: string | null; return_reason?: string | null; tracking_url?: string | null }
...
export interface StoreSettings { id?: number; configured?: boolean; store_name: string; support_phone?: string | null; support_email?: string | null; store_address?: string | null; currency: string; shipping_fee: number; free_shipping_from: number; low_stock_threshold: number; order_prefix: string; cod_enabled: boolean; bank_transfer_enabled: boolean; bank_name?: string | null; bank_account_name?: string | null; bank_account_number?: string | null; bank_branch?: string | null; bank_qr_path?: string | null; bank_transfer_note?: string | null; returns_enabled: boolean; return_window_days: number; exchange_enabled: boolean; exchange_window_days: number; refund_shipping_on_full_return: boolean; warranty_enabled: boolean; appointments_enabled: boolean; appointment_cancel_before_hours: number; store_timezone: string | null; hair_finder_config?: Record<string, unknown> | null }
export interface ContentPage { id?: number; page_key: string; title: string; summary?: string | null; content?: { intro?: string; sections?: Array<{ title: string; body?: string; items?: string[] }> } | null; is_active: boolean; seo_title?: string | null; seo_description?: string | null; published_at?: string | null; business_rules?: Record<string, boolean | number> }
