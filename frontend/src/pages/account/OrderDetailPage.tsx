...
export function OrderDetailPage() {
  ...
  return <div className='card p-6'>
    <div className='flex flex-wrap items-start justify-between gap-4'><div><div className='muted'>Mã đơn</div><div className='flex items-center gap-2'><h1 className='text-2xl font-black'>{order.order_number}</h1><button type='button' aria-label='Sao chép mã đơn' onClick={() => void copyText(order.order_number).then(() => toast.success('Đã sao chép'))}><Clipboard size={17} /></button></div></div><div className='flex flex-wrap items-center gap-2'><button className='btn-secondary' disabled={pendingAction !== null} onClick={() => void buyAgain()}><RotateCcw size={17} />{pendingAction === 'buy-again' ? 'Đang thêm...' : 'Mua lại'}</button><span className='rounded-full bg-emerald-50 px-3 py-2 font-bold text-emerald-800'>{statusLabel[order.order_status]}</span></div></div>
