...
export function CheckoutPage() {
  ...
  const codEnabled = paymentMethods.data?.cod.enabled === true
  const bankTransferEnabled = paymentMethods.data?.bank_transfer.enabled === true
  const defaultPaymentMethod = codEnabled ? 'cod' : bankTransferEnabled ? 'bank_transfer' : ''
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const paymentMethod = String(form.get('payment_method') ?? defaultPaymentMethod)
    if (!paymentMethod) { toast.error('Cửa hàng chưa bật phương thức thanh toán.'); return }
    setSubmitting(true)
    const data = { ...shipping, customer_note: String(form.get('customer_note') ?? ''), payment_method: paymentMethod }
    try {
  ...
  return <div className='container-page py-10'><h1 className='section-title'>Thanh toán</h1><form className='mt-7 grid gap-6 lg:grid-cols-[1fr_420px]' onSubmit={submit}>
    <section className='card p-6'><h2 className='text-xl font-black'>THÔNG TIN THANH TOÁN</h2>
      ...
      <h2 className='mt-8 text-xl font-black'>PHƯƠNG THỨC THANH TOÁN</h2><div className='mt-4 grid gap-3'>{paymentMethods.isLoading && <p className='muted'>Đang tải phương thức thanh toán...</p>}{codEnabled && <label className='rounded-xl border p-4'><input type='radio' name='payment_method' value='cod' defaultChecked={defaultPaymentMethod === 'cod'} /> <strong>Thanh toán khi nhận hàng (COD)</strong></label>}{bankTransferEnabled && <label className='rounded-xl border p-4'><input type='radio' name='payment_method' value='bank_transfer' defaultChecked={defaultPaymentMethod === 'bank_transfer'} /> <strong>Chuyển khoản ngân hàng</strong><span className='muted mt-1 block text-sm'>{paymentMethods.data?.bank_transfer.bank_name} · {paymentMethods.data?.bank_transfer.account_number}</span></label>}{!paymentMethods.isLoading && !defaultPaymentMethod && <p className='rounded-xl bg-amber-50 p-4 text-sm font-semibold text-amber-900'>Cửa hàng chưa bật phương thức thanh toán.</p>}</div>
    </section>
    <aside className='card h-fit p-6'><h2 className='text-xl font-black'>THÔNG TIN ĐƠN HÀNG</h2><div className='mt-4 grid gap-4'>{items.map((item) => <article key={item.id} className='flex gap-3 border-b pb-4'><img src={item.variant.product.images?.[0]?.image_path || '/images/product-placeholder.svg'} alt={item.variant.product.name} className='h-20 w-20 rounded-xl object-cover' /><div className='min-w-0 flex-1'><strong>{item.variant.product.name}</strong>{item.variant.attributes.map((attribute) => <div className='mt-1 text-xs text-slate-500' key={attribute.attribute_id}>{attribute.attribute_name || 'Tùy chọn'}: {attribute.value}{attribute.option_code ? ' (' + attribute.option_code + ')' : ''}</div>)}<div className='mt-2 text-sm'>{formatPrice(item.unit_price)} × {item.quantity}</div></div></article>)}</div><div className='mt-5 flex gap-2'><input className='input' value={coupon} onChange={(event) => setCoupon(event.target.value.toUpperCase())} placeholder='Mã giảm giá' /><button type='button' className='btn-secondary' onClick={() => void preview()}>Áp dụng</button></div><div className='mt-5 grid gap-3 border-t pt-5'><div className='flex justify-between'><span>Tạm tính</span><strong>{formatPrice(summary.subtotal)}</strong></div><div className='flex justify-between'><span>Giảm giá</span><strong>-{formatPrice(summary.discount_amount)}</strong></div><div className='flex justify-between'><span>Phí giao hàng</span><strong>{formatPrice(summary.shipping_fee)}</strong></div><div className='flex justify-between border-t pt-4 text-lg'><strong>Tổng cộng</strong><strong className='price'>{formatPrice(summary.total_amount)}</strong></div></div><button className='btn-primary mt-5 w-full' disabled={submitting || !defaultPaymentMethod}>{submitting ? 'Đang đặt hàng...' : 'THANH TOÁN'}</button></aside>
  </form></div>
}
