...
export function CartPage() {
  ...
  const checkout = () => navigate('/thanh-toan')
  const freeFrom = settings.data?.shipping?.free_from ?? 0
  const freeShippingRemaining = freeFrom > 0 ? Math.max(0, freeFrom - subtotal) : 0
  return <div className="container-page py-10"><h1 className="section-title">Giỏ hàng</h1><p className="muted mt-2">Kiểm tra sản phẩm và số lượng trước khi thanh toán.</p>
    {!items.length ? <div className="mt-8"><EmptyState title="Giỏ hàng đang trống" description="Hãy chọn sản phẩm phù hợp để bắt đầu mua sắm." /><div className="mt-4 text-center"><Link to="/san-pham" className="btn-primary"><ShoppingBag size={18} />Xem sản phẩm</Link></div></div> : <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
      ...
      <aside className="card h-fit p-6"><h2 className="text-xl font-black">Tóm tắt đơn hàng</h2>{freeFrom > 0 && <div className='mt-4 rounded-xl bg-emerald-50 p-3 text-sm'><strong>{freeShippingRemaining > 0 ? `Mua thêm ${formatPrice(freeShippingRemaining)} để được miễn phí vận chuyển` : 'Bạn đã đủ điều kiện miễn phí vận chuyển'}</strong><div className='mt-2 h-2 overflow-hidden rounded-full bg-white'><div className='h-full bg-emerald-700' style={{ width: `${Math.min(100, subtotal / freeFrom * 100)}%` }} /></div></div>}<div className="mt-5 flex justify-between"><span className="muted">Tạm tính</span><strong>{formatPrice(subtotal)}</strong></div><div className="mt-3 flex justify-between"><span className="muted">Phí giao hàng</span><span>{freeFrom > 0 && subtotal >= freeFrom ? 'Miễn phí' : 'Tính khi thanh toán'}</span></div><div className="mt-5 border-t pt-5"><div className="flex justify-between text-lg"><strong>Tổng dự kiến</strong><strong className="price">{formatPrice(subtotal)}</strong></div><button className="btn-primary mt-5 w-full" onClick={checkout}>Tiến hành thanh toán</button><Link to="/san-pham" className="btn-secondary mt-3 w-full">Tiếp tục mua sắm</Link></div></aside>
    </div>}
  </div>
}
