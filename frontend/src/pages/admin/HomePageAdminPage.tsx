...
type SectionConfig = { key: SectionKey; title: string; description: string; cropLabel?: string; fields: FieldConfig[]; list?: ListConfig }

const sectionConfigs: SectionConfig[] = [
  { key: 'hero', title: 'Nội dung Hero', description: 'Khối banner lớn đầu trang.', cropLabel: 'Cắt ảnh 16:9', fields: [{ key: 'eyebrow', label: 'Nhãn nhỏ Hero' }, { key: 'title', label: 'Tiêu đề Hero' }, { key: 'description', label: 'Mô tả Hero', multiline: true }, { key: 'primary_label', label: 'Nhãn nút chính' }, { key: 'primary_url', label: 'Đường dẫn nút chính' }, { key: 'secondary_label', label: 'Nhãn nút phụ' }, { key: 'secondary_url', label: 'Đường dẫn nút phụ' }, { key: 'note_label', label: 'Nhãn ghi chú trên ảnh' }, { key: 'note_value', label: 'Nội dung ghi chú trên ảnh' }], list: { key: 'trust_items', label: 'Các cam kết dưới Hero', columns: ['value'], hint: 'Mỗi dòng là một cam kết.' } },
  ...
  { key: 'brand_story', title: 'Câu chuyện thương hiệu', description: 'Thông điệp và các giá trị thương hiệu.', cropLabel: 'Cắt ảnh 1:1', fields: [{ key: 'kicker', label: 'Nhãn nhỏ' }, { key: 'title', label: 'Tiêu đề' }, { key: 'description', label: 'Mô tả', multiline: true }, { key: 'cta_label', label: 'Nhãn nút' }, { key: 'cta_url', label: 'Đường dẫn nút' }], list: { key: 'values', label: 'Giá trị thương hiệu', columns: ['title', 'description'], hint: 'Mỗi dòng: Tiêu đề | Mô tả' } },
  { key: 'solutions', title: 'Giải pháp dành cho bạn', description: 'Khối giới thiệu lợi ích, hướng dẫn và ảnh minh họa.', cropLabel: 'Cắt ảnh 6:5', fields: [{ key: 'kicker', label: 'Nhãn nhỏ' }, { key: 'title', label: 'Tiêu đề' }, { key: 'description', label: 'Mô tả', multiline: true }, { key: 'cta_label', label: 'Nhãn nút' }, { key: 'cta_url', label: 'Đường dẫn nút' }, { key: 'art_text', label: 'Chữ trên ảnh', multiline: true }], list: { key: 'bullets', label: 'Các lợi ích', columns: ['value'], hint: 'Mỗi dòng là một lợi ích.' } },
  { key: 'styles', title: 'Cảm hứng phong cách', description: 'Mỗi thẻ phong cách có nội dung, đường dẫn và ảnh riêng.', cropLabel: 'Cắt từng ảnh 4:3', fields: [{ key: 'kicker', label: 'Nhãn nhỏ' }, { key: 'title', label: 'Tiêu đề' }], list: { key: 'items', label: 'Các phong cách', columns: ['title', 'description', 'url'], hint: 'Mỗi dòng: Tiêu đề | Mô tả | Đường dẫn' } },
  { key: 'process', title: 'Quy trình', description: 'Mỗi bước tư vấn và chăm sóc có ảnh riêng.', cropLabel: 'Cắt từng ảnh 16:9', fields: [{ key: 'kicker', label: 'Nhãn nhỏ' }, { key: 'title', label: 'Tiêu đề' }, { key: 'description', label: 'Mô tả', multiline: true }, { key: 'cta_label', label: 'Nhãn nút' }, { key: 'cta_url', label: 'Đường dẫn nút' }], list: { key: 'steps', label: 'Các bước', columns: ['number', 'title', 'description'], hint: 'Mỗi dòng: Số thứ tự | Tiêu đề | Mô tả' } },
  { key: 'testimonials', title: 'Cảm nhận khách hàng', description: 'Mỗi cảm nhận có ảnh và bài viết chi tiết mở khi khách hàng bấm vào.', cropLabel: 'Cắt từng ảnh 16:9', fields: [{ key: 'kicker', label: 'Nhãn nhỏ' }, { key: 'title', label: 'Tiêu đề' }], list: { key: 'items', label: 'Các cảm nhận', columns: ['quote', 'customer', 'label'], hint: 'Mỗi dòng: Nội dung ngắn | Tên khách hàng | Nhãn' } },
  { key: 'contact', title: 'Kết nối', description: 'Khối lựa chọn cách liên hệ.', fields: [{ key: 'kicker', label: 'Nhãn nhỏ' }, { key: 'title', label: 'Tiêu đề' }, { key: 'description', label: 'Mô tả', multiline: true }], list: { key: 'cards', label: 'Các lựa chọn kết nối', columns: ['title', 'description', 'url'], hint: 'Mỗi dòng: Tiêu đề | Mô tả | Đường dẫn' } },
...
export function HomePageAdminPage() {
  ...
  const updateSectionList = (section: SectionKey, listKey: string, value: string, columns: string[]) => setDraft((current) => {
    if (!current) return current
    const sectionValue = current.sections[section] as unknown as Record<string, unknown>
    const parsedItems = parseList(value, columns)
    const currentItems = Array.isArray(sectionValue[listKey]) ? sectionValue[listKey] as Record<string, unknown>[] : []
    const items = columns.length === 1
      ? parsedItems
      : parsedItems.map((item, index) => ({ ...currentItems[index], ...(item as Record<string, unknown>) }))
    ...
    <form className="grid gap-5" onSubmit={save}>
      ...
      {sectionConfigs.map((config) => {
        ...
        return <details className="card group p-6" key={config.key} open={config.key === 'hero'}>
          <summary className="cursor-pointer list-none"><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-xl font-black">{config.title}</h2><p className="muted mt-1">{config.description}</p></div>{config.cropLabel && <span className="rounded-full bg-rose-50 px-3 py-1 text-xs font-black text-rose-800">{config.cropLabel}</span>}</div></summary>
          {config.key === 'hero' && <HomeImageEditor mediaKey="hero" title="Hero" description="Ảnh riêng của khung lớn đầu trang. JPG, PNG hoặc WebP, tối đa 5 MB." path={draft.hero_image_path} alt={draft.hero_image_alt ?? ''} fallback="/images/product-placeholder.svg" uploading={uploading} objectPositionX={draft.sections.hero.image_position_x ?? 50} objectPositionY={draft.sections.hero.image_position_y ?? 50} onObjectPositionChange={(axis, value) => updateSection('hero', axis === 'x' ? 'image_position_x' : 'image_position_y', value)} onAltChange={(value) => updateRoot('hero_image_alt', value)} onUpload={(file) => { void uploadImage('hero', file) }} onRemove={() => { void removeImage('hero') }} />}
          {config.key === 'brand_story' && <HomeImageEditor mediaKey="brandStory" title="Câu chuyện thương hiệu" description="Ảnh minh họa nằm bên trái nội dung Câu chuyện thương hiệu trên trang chủ." path={draft.brand_story_image_path} alt={String(section.image_alt ?? '')} fallback="/images/product-placeholder.svg" uploading={uploading} objectPositionX={draft.sections.brand_story.image_position_x ?? 50} objectPositionY={draft.sections.brand_story.image_position_y ?? 50} onObjectPositionChange={(axis, value) => updateSection('brand_story', axis === 'x' ? 'image_position_x' : 'image_position_y', value)} onAltChange={(value) => updateSection('brand_story', 'image_alt', value)} onUpload={(file) => { void uploadImage('brand_story', file) }} onRemove={() => { void removeImage('brand_story') }} />}
          {config.key === 'solutions' && <HomeImageEditor mediaKey="solution" title="Giải pháp dành cho bạn" description="Ảnh minh họa bên phải nội dung giải pháp, ưu tiên giữ trọn tóc và khuôn mặt." path={String(section.image_path ?? '') || null} alt={String(section.image_alt ?? '')} fallback="/images/product-placeholder.svg" uploading={uploading} objectPositionX={draft.sections.solutions.image_position_x ?? 50} objectPositionY={draft.sections.solutions.image_position_y ?? 50} onObjectPositionChange={(axis, value) => updateSection('solutions', axis === 'x' ? 'image_position_x' : 'image_position_y', value)} onAltChange={(value) => updateSection('solutions', 'image_alt', value)} onUpload={(file) => { void uploadSectionImage('solutions', 'Giải pháp dành cho bạn', file) }} onRemove={() => { void removeSectionImage('solutions', 'Giải pháp dành cho bạn') }} />}
          {config.key === 'styles' && (draft.sections.styles.items ?? []).map((item, index) => <HomeImageEditor key={`style-image-${index}`} mediaKey="inspiration" title={`Phong cách ${index + 1}`} description={`Ảnh cho thẻ “${item.title}”.`} path={item.image_path} alt={item.image_alt ?? ''} fallback="/images/product-placeholder.svg" uploading={uploading} objectPositionX={item.image_position_x ?? 50} objectPositionY={item.image_position_y ?? 50} onObjectPositionChange={(axis, value) => updateListItem('styles', 'items', index, axis === 'x' ? 'image_position_x' : 'image_position_y', value)} onAltChange={(value) => updateListItem('styles', 'items', index, 'image_alt', value)} onUpload={(file) => { void uploadSectionImage('styles', `Phong cách ${index + 1}`, file, index) }} onRemove={() => { void removeSectionImage('styles', `Phong cách ${index + 1}`, index) }} />)}
          {config.key === 'process' && (draft.sections.process.steps ?? []).map((step, index) => <HomeImageEditor key={`process-image-${index}`} mediaKey="process" title={`Bước ${step.number || index + 1}`} description={`Ảnh cho bước “${step.title}”.`} path={step.image_path} alt={step.image_alt ?? ''} fallback="/images/product-placeholder.svg" uploading={uploading} objectPositionX={step.image_position_x ?? 50} objectPositionY={step.image_position_y ?? 50} onObjectPositionChange={(axis, value) => updateListItem('process', 'steps', index, axis === 'x' ? 'image_position_x' : 'image_position_y', value)} onAltChange={(value) => updateListItem('process', 'steps', index, 'image_alt', value)} onUpload={(file) => { void uploadSectionImage('process', `Bước ${step.number || index + 1}`, file, index) }} onRemove={() => { void removeSectionImage('process', `Bước ${step.number || index + 1}`, index) }} />)}
          {config.key === 'testimonials' && (draft.sections.testimonials.items ?? []).map((item, index) => <div className="mt-5 grid gap-4 rounded-3xl border border-slate-200 p-4" key={`testimonial-editor-${index}`}><HomeImageEditor mediaKey="testimonial" title={`Cảm nhận ${index + 1}`} description={`Ảnh đại diện cho cảm nhận của ${item.customer}.`} path={item.image_path} alt={item.image_alt ?? ''} fallback="/images/product-placeholder.svg" uploading={uploading} objectPositionX={item.image_position_x ?? 50} objectPositionY={item.image_position_y ?? 50} onObjectPositionChange={(axis, value) => updateListItem('testimonials', 'items', index, axis === 'x' ? 'image_position_x' : 'image_position_y', value)} onAltChange={(value) => updateListItem('testimonials', 'items', index, 'image_alt', value)} onUpload={(file) => { void uploadSectionImage('testimonials', `Cảm nhận ${index + 1}`, file, index) }} onRemove={() => { void removeSectionImage('testimonials', `Cảm nhận ${index + 1}`, index) }} /><label><span className="label">Tiêu đề bài cảm nhận {index + 1}</span><input className="input" value={item.detail_title ?? ''} onChange={(event) => updateListItem('testimonials', 'items', index, 'detail_title', event.target.value)} /></label><label><span className="label">Nội dung chi tiết cảm nhận {index + 1}</span><textarea className="input min-h-40" value={item.detail ?? ''} onChange={(event) => updateListItem('testimonials', 'items', index, 'detail', event.target.value)} /></label></div>)}
