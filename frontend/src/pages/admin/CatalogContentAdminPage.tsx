...
export function CatalogContentAdminPage() {
  ...
  const publicPath = pageKey === 'products' ? '/san-pham' : isHairGuide ? '/huong-dan-chon-toc' : `/danh-muc/${index.data?.categories.find((item) => `category-${item.id}` === pageKey)?.slug ?? ''}`
  ...
  return <form onSubmit={save} noValidate>
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-3xl font-black">{isHairGuide ? 'Nội dung Hair Guide' : 'Nội dung trang sản phẩm'}</h1><p className="muted">Quản trị hero, nội dung biên tập, tư vấn và SEO của catalog.</p></div><a className="btn-secondary" href={publicPath} target="_blank" rel="noreferrer"><ExternalLink size={17} />Mở trang public</a></div>

    <section className="card p-5"><label><span className="label">Trang cần chỉnh</span><select className="input max-w-md" value={pageKey} onChange={(event) => setPageKey(event.target.value)}><option value="products">Trang tổng /san-pham</option><option value="hair-guide">Hướng dẫn chọn tóc — /huong-dan-chon-toc</option>{index.data?.categories.map((item) => <option key={item.id} value={`category-${item.id}`}>Danh mục: {item.name}</option>)}</select></label></section>
