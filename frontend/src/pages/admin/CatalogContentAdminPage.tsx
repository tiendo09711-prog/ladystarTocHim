...
import { HomeImageCropEditor as CatalogImageEditor } from '../../components/admin/HomeImageCropEditor'
import type { HomeMediaKey } from '../../config/homeMedia'
...
export function CatalogContentAdminPage() {
  ...
  const [uploadingSlot, setUploadingSlot] = useState<'hero' | 'consultation' | null>(null)
  ...
  const upload = async (slot: 'hero' | 'consultation', file?: File) => {
    ...
    form.set('slot', slot)
    form.set('image', file)
    try { setUploadingSlot(slot); await apiClient.post(`/admin/catalog/content/${pageKey}/images`, form); toast.success(`Đã cập nhật ảnh ${slot === 'hero' ? 'Hero' : 'Tư vấn'}.`); await detail.refetch() } catch (error) { setApiErrors(error) } finally { setUploadingSlot(null) }
  }
  const removeImage = async (slot: 'hero' | 'consultation') => {
    try { setUploadingSlot(slot); await apiClient.delete(`/admin/catalog/content/${pageKey}/images`, { data: { slot } }); toast.success('Đã xóa ảnh.'); await detail.refetch() } catch (error) { setApiErrors(error) } finally { setUploadingSlot(null) }
  }
  ...
  const heroMediaKey: HomeMediaKey = isHairGuide ? 'guideHero' : 'catalogHero'
  const consultationMediaKey: HomeMediaKey = isHairGuide ? 'guideConsultation' : 'catalogConsultation'
  ...
  return <form onSubmit={save} noValidate>
    ...
    {isHairGuide && <section className="card mt-5 flex flex-wrap items-center justify-between gap-3 border-emerald-200 bg-emerald-50 p-5"><div><h2 className="text-lg font-black">Danh sách dịch vụ được quản lý riêng</h2><p className="text-sm text-slate-600">Trang này chỉ chỉnh hero, nội dung và SEO. Dữ liệu dịch vụ lấy từ bảng services.</p></div><Link className="btn-primary" to="/admin/services">Mở quản lý dịch vụ</Link></section>}

    <section className="card mt-5 p-5"><h2 className="text-xl font-black">Hero</h2><div className="mt-4 grid gap-4 md:grid-cols-2"><label><span className="label">Eyebrow</span><input className="input" value={content.eyebrow ?? ''} onChange={(event) => update('eyebrow', event.target.value)} /></label><label><span className="label">Badge</span><input className="input" value={content.settings_json?.hero_badge ?? ''} onChange={(event) => updateSettings({ hero_badge: event.target.value })} /></label><label><span className="label">Tiêu đề</span><input className="input" value={content.title ?? ''} onChange={(event) => update('title', event.target.value)} /></label><label className="md:col-span-2"><span className="label">Subtitle</span><textarea className="input min-h-24" value={content.subtitle ?? ''} onChange={(event) => update('subtitle', event.target.value)} /></label></div><CatalogImageEditor mediaKey={heroMediaKey} title={`Hero ${isHairGuide ? 'Dịch vụ chăm sóc' : 'Catalog'}`} description="Ảnh fill đầy khung, không kéo giãn; kéo và phóng to để giữ đúng vùng cần hiển thị." path={content.hero_image_path} alt={content.hero_image_alt ?? ''} fallback="/images/product-placeholder.svg" uploading={uploadingSlot === 'hero'} onAltChange={(value) => update('hero_image_alt', value)} onUpload={(file) => { void upload('hero', file) }} onRemove={() => { void removeImage('hero') }} /></section>
    ...
    <section className="card mt-5 p-5"><h2 className="text-xl font-black">Tư vấn</h2><div className="mt-4 grid gap-4 md:grid-cols-2"><label><span className="label">Tiêu đề</span><input className="input" value={content.consultation_title ?? ''} onChange={(event) => update('consultation_title', event.target.value)} /></label><label><span className="label">CTA</span><input className="input" value={content.consultation_cta_label ?? ''} onChange={(event) => update('consultation_cta_label', event.target.value)} /></label><label><span className="label">Benefits, mỗi dòng một ý</span><textarea className="input" value={(content.settings_json?.consultation_benefits ?? []).join('\n')} onChange={(event) => updateSettings({ consultation_benefits: event.target.value.split('\n').map((item) => item.trim()).filter(Boolean) })} /></label><label className="md:col-span-2"><span className="label">Nội dung</span><textarea className="input min-h-24" value={content.consultation_body ?? ''} onChange={(event) => update('consultation_body', event.target.value)} /></label></div><CatalogImageEditor mediaKey={consultationMediaKey} title={`Tư vấn ${isHairGuide ? 'Dịch vụ chăm sóc' : 'Catalog'}`} description="Ảnh fill đầy khung tư vấn và giữ bố cục ổn định trên desktop lẫn mobile." path={content.consultation_image_path} alt={content.consultation_image_alt ?? ''} fallback="/images/product-placeholder.svg" uploading={uploadingSlot === 'consultation'} onAltChange={(value) => update('consultation_image_alt', value)} onUpload={(file) => { void upload('consultation', file) }} onRemove={() => { void removeImage('consultation') }} /></section>
