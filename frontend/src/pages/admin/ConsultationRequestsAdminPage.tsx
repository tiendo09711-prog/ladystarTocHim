...
export function ConsultationRequestsAdminPage() {
  ...
  return <div>
    <div className="mb-6 flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-3xl font-black">Yêu cầu tư vấn</h1><p className="muted">Theo dõi và cập nhật yêu cầu từ các trang tư vấn.</p></div><label className="min-w-52"><span className="label">Lọc trạng thái</span><select className="input" value={filterStatus} onChange={(event) => setFilterStatus(event.target.value)}>{statuses.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div>
    <div className="table-wrap"><table className="table"><thead><tr><th>Khách hàng</th><th>Nguồn</th><th>Dịch vụ / nhu cầu</th><th>Thời gian</th><th>Trạng thái & ghi chú</th></tr></thead><tbody>
      {query.data?.data.map((item) => <tr key={item.id}>
        <td><strong>{item.name}</strong><br /><span className="text-sm text-slate-500">{item.phone}</span></td>
