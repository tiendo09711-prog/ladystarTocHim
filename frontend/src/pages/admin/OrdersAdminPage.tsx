...
import { CheckCircle2, Clipboard, CreditCard, MoreHorizontal, PackageCheck, Printer } from 'lucide-react'
...
import { adminAttentionQueryKey } from '../../features/admin/useAdminAttention'
...
export function OrdersAdminPage() {
  ...
  const refresh = () => Promise.all([client.invalidateQueries({ queryKey: ['admin-orders'] }), client.invalidateQueries({ queryKey: adminAttentionQueryKey }), client.invalidateQueries({ queryKey: ['admin-summary'] })])
  const runAction = async (key: string, request: () => Promise<unknown>, message: string) => { setPendingAction(key); try { await request(); await refresh(); toast.success(message) } catch (error) { toast.error(apiError(error, 'Không thể thực hiện thao tác.')) } finally { setPendingAction(null) } }

  return <div><div className='mb-6'><h1 className='text-3xl font-black'>Đơn hàng</h1><p className='muted'>Theo dõi và xử lý vòng đời đơn hàng.</p></div><div className='card mb-5 grid gap-3 p-4 md:grid-cols-3'><input className='input' placeholder='Mã đơn, khách hàng, điện thoại' value={params.get('search') ?? ''} onChange={(event) => update('search', event.target.value)} /><select className='input' value={params.get('status') ?? ''} onChange={(event) => update('status', event.target.value)}><option value=''>Tất cả trạng thái</option>{['pending', 'confirmed', 'processing', 'shipping', 'completed', 'cancelled'].map((item) => <option key={item} value={item}>{statusLabel[item]}</option>)}</select><select className='input' value={params.get('payment_status') ?? ''} onChange={(event) => update('payment_status', event.target.value)}><option value=''>Tất cả thanh toán</option><option value='unpaid'>Chưa thanh toán</option><option value='paid'>Đã thanh toán</option></select></div>{query.isLoading ? <LoadingState /> : <div className='table-wrap'><table className='table'><thead><tr><th>Mã đơn</th><th>Khách hàng</th><th>Tổng tiền</th><th>Thanh toán</th><th>Trạng thái</th><th>Ngày đặt</th><th></th></tr></thead><tbody>{query.data?.data.map((order) => <tr key={order.id}><td><div className='flex items-center gap-2 font-bold'>{order.order_number}<button aria-label='Sao chép mã đơn' onClick={() => void copyText(order.order_number).then(() => toast.success('Đã sao chép'))}><Clipboard size={14} /></button></div></td><td>{order.customer_name}</td><td className='price'>{formatPrice(order.total_amount)}</td><td>{statusLabel[order.payment_status]}</td><td>{statusLabel[order.order_status]}</td><td>{new Date(order.created_at).toLocaleDateString('vi-VN')}</td><td><details className='relative'><summary className='cursor-pointer list-none rounded-lg p-2 hover:bg-slate-100' aria-label={`Thao tác đơn ${order.order_number}`}><MoreHorizontal size={18} /></summary><div className='absolute right-0 z-20 mt-1 grid min-w-52 gap-1 rounded-xl border bg-white p-2 shadow-xl'><Link className='rounded-lg px-3 py-2 text-sm font-bold hover:bg-slate-50' to={`/admin/orders/${order.id}`}>Xem chi tiết</Link><button className='flex items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-bold hover:bg-slate-50' onClick={() => void copyText(order.order_number).then(() => toast.success('Đã sao chép'))}><Clipboard size={15} />Copy mã đơn</button>{can(user, 'orders.status.manage') && order.order_status === 'pending' && <button className='flex items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-bold text-emerald-800 hover:bg-emerald-50' disabled={pendingAction === `confirm-${order.id}`} onClick={() => void runAction(`confirm-${order.id}`, () => apiClient.patch(`/admin/orders/${order.id}/status`, { order_status: 'confirmed' }), 'Đã xác nhận đơn hàng.')}><CheckCircle2 size={15} />Xác nhận đơn</button>}{can(user, 'orders.payment.manage') && order.payment_status === 'unpaid' && order.order_status !== 'cancelled' && <button className='flex items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-bold text-emerald-800 hover:bg-emerald-50' disabled={pendingAction === `pay-${order.id}`} onClick={() => void runAction(`pay-${order.id}`, () => apiClient.patch(`/admin/orders/${order.id}/payment-status`, { payment_status: 'paid' }), 'Đã xác nhận thanh toán.')}><CreditCard size={15} />Xác nhận thanh toán</button>}{can(user, 'orders.shipment.manage') && ['confirmed', 'processing'].includes(order.order_status) && <Link className='rounded-lg px-3 py-2 text-sm font-bold hover:bg-slate-50' to={`/admin/orders/${order.id}`}>Xử lý giao hàng</Link>}</div></details></td></tr>)}</tbody></table></div>}</div>
}
