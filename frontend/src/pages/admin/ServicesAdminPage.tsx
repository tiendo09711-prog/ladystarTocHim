    ...
    <form className="card mb-6 grid gap-5 p-5" onSubmit={save} noValidate>
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_280px]">
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label><span className="label">Giá dịch vụ</span><input className="input" type="number" min="0" step="1000" value={form.price} onChange={(event) => setForm((current) => ({ ...current, price: Number(event.target.value) }))} required />{fieldError('price') && <small className="text-red-700">{fieldError('price')}</small>}</label>
