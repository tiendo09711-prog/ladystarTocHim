...
import { HairFinderConfigEditor } from '../../components/admin/HairFinderConfigEditor'
...
export function SettingsAdminPage() {
  const { user } = useAuth(); const editable = can(user, 'settings.manage')
  ...
  const save = async (event: FormEvent<HTMLFormElement>) => {
    ...
    let hairFinderConfig: Record<string, unknown> | null = null
    const hairFinderRaw = String(form.get('hair_finder_config') ?? '').trim()
    if (hairFinderRaw) {
      try { hairFinderConfig = JSON.parse(hairFinderRaw) as Record<string, unknown> } catch { toast.error('Cấu hình Hair Finder không phải JSON hợp lệ.'); return }
    }
    const payload = {
      store_name: String(form.get('store_name') ?? ''), support_phone: nullable('support_phone'), support_email: nullable('support_email'), store_address: nullable('store_address'), currency: String(form.get('currency') ?? '').trim().toUpperCase(),
      ...
      cod_enabled: enabled('cod_enabled'), bank_transfer_enabled: enabled('bank_transfer_enabled'), bank_name: nullable('bank_name'), bank_account_name: nullable('bank_account_name'), bank_account_number: nullable('bank_account_number'), bank_branch: nullable('bank_branch'), bank_transfer_note: nullable('bank_transfer_note'),
      ...
      hair_finder_config: hairFinderConfig,
    }
    try { await apiClient.put('/admin/settings', payload); await Promise.all([client.invalidateQueries({ queryKey: ['admin-settings'] }), client.invalidateQueries({ queryKey: ['public-settings'] })]); toast.success('Đã lưu cấu hình cửa hàng.') } catch { toast.error('Không thể lưu cấu hình. Vui lòng kiểm tra dữ liệu.') }
  }
  ...
  return <div>
    <div className='mb-6'><h1 className='text-3xl font-black'>Cài đặt cửa hàng</h1><p className='muted'>Cấu hình checkout, hậu mãi, bảo hành, lịch hẹn và Hair Finder.</p>{settings.configured === false && <p className='mt-3 rounded-xl bg-amber-50 p-3 font-bold text-amber-900'>Cấu hình cửa hàng chưa được thiết lập. Hoàn tất các trường bắt buộc rồi lưu để bật các luồng business.</p>}</div>
    <form className='grid gap-6' onSubmit={editable ? save : (event) => event.preventDefault()}>
      <section className='card grid gap-5 p-6 md:grid-cols-2'><h2 className='text-xl font-black md:col-span-2'>Thông tin chung</h2><Field name='store_name' label='Tên cửa hàng' value={settings.store_name} required /><Field name='currency' label='Mã tiền tệ ISO' value={settings.currency} required /><Field name='order_prefix' label='Tiền tố mã đơn' value={settings.order_prefix} required /><Field name='support_phone' label='Điện thoại hỗ trợ' value={settings.support_phone} /><Field name='support_email' label='Email hỗ trợ' value={settings.support_email} type='email' /><Field name='store_address' label='Địa chỉ cửa hàng' value={settings.store_address} wide /><Field name='shipping_fee' label='Phí giao hàng' value={settings.shipping_fee} type='number' required /><Field name='free_shipping_from' label='Ngưỡng miễn phí giao hàng' value={settings.free_shipping_from} type='number' required /><Field name='low_stock_threshold' label='Ngưỡng cảnh báo tồn kho' value={settings.low_stock_threshold} type='number' required /></section>
      <section className='card grid gap-5 p-6 md:grid-cols-2'><div className='md:col-span-2'><h2 className='text-xl font-black'>Phương thức thanh toán</h2><div className='mt-3 flex flex-wrap gap-5'><Toggle name='cod_enabled' label='Bật thanh toán COD' checked={settings.cod_enabled} /><Toggle name='bank_transfer_enabled' label='Bật chuyển khoản ngân hàng' checked={settings.bank_transfer_enabled} /></div></div><Field name='bank_name' label='Tên ngân hàng' value={settings.bank_name} /><Field name='bank_account_name' label='Chủ tài khoản' value={settings.bank_account_name} /><Field name='bank_account_number' label='Số tài khoản' value={settings.bank_account_number} /><Field name='bank_branch' label='Chi nhánh' value={settings.bank_branch} /><Field name='bank_transfer_note' label='Hướng dẫn chuyển khoản' value={settings.bank_transfer_note} wide /></section>
      ...
      <HairFinderConfigEditor initialConfig={settings.hair_finder_config} editable={editable} />
      {editable && <button className='btn-primary justify-self-start'><Save size={18} />Lưu cấu hình</button>}
    </form>
    <section className='card mt-6 p-6'><h2 className='text-xl font-black'>Ảnh QR chuyển khoản</h2>{settings.bank_qr_path ? <div className='mt-4 flex flex-wrap items-end gap-4'><img className='max-h-64 rounded-xl border object-contain p-2' src={resolveAssetUrl(settings.bank_qr_path)} alt='QR chuyển khoản hiện tại' />{editable && <button className='btn-secondary text-red-700' type='button' onClick={() => void deleteQr()}><Trash2 size={17} />Xóa QR</button>}</div> : <p className='muted mt-3'>Chưa có ảnh QR.</p>}{editable && <form className='mt-4 flex flex-wrap items-end gap-3' onSubmit={uploadQr}><label><span className='label'>Chọn ảnh JPG, PNG hoặc WEBP</span><input className='input' type='file' name='image' accept='image/jpeg,image/png,image/webp' required /></label><button className='btn-secondary'><Upload size={17} />Upload QR</button></form>}</section>
  </div>
}
