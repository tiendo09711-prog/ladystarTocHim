...
test('đặt lịch tư vấn mở popup và xác nhận thành công', async ({ page }) => {
  ...
  await page.route('**/api/v1/settings/public', async (route) => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ success: true, data: { configured: true, store_name: 'Cửa hàng kiểm thử', appointments_enabled: true } }),
  }))
  ...
  const dialog = page.getByRole('dialog', { name: /Gửi yêu cầu tư vấn/ })
  await expect(dialog).toBeVisible()
  await expect(dialog.locator('.consultation-dialog-media')).toHaveCount(0)
