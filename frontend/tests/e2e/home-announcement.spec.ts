...
test('admin chỉnh sửa và lưu cấu hình thanh thông báo', async ({ page }) => {
  ...
  await expect(page.getByText('Quản trị', { exact: true }).first()).toBeVisible()
...
test('admin cập nhật nội dung và các ảnh riêng của trang chủ', async ({ page }) => {
  ...
  const processDetails = page.locator('details').filter({ hasText: 'Quy trình' })
  ...
  const restoredProcessDetails = page.locator('details').filter({ hasText: 'Quy trình' })
