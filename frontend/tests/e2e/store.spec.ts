...
test('khách khám phá trang chủ và thêm vào giỏ', async ({ page }) => {
  ...
  await page.route('**/api/v1/settings/public', (route) => route.fulfill({ json: { success: true, data: { configured: true, store_name: 'Cửa hàng kiểm thử', currency: 'VND', appointments_enabled: true } } }))
  await page.goto('/')
  ...
  const productMenu = page.getByRole('button', { name: 'Sản phẩm', exact: true })
  await productMenu.hover()
  await expect(productMenu.locator('xpath=..').locator('a[href^="/danh-muc/"]').first()).toBeVisible()
  await page.getByRole('button', { name: 'Tin tức & ưu đãi' }).hover()
  ...
  await page.goBack()
  const categoryLink = page.getByRole('button', { name: 'Sản phẩm', exact: true }).locator('xpath=..').locator('a[href^="/danh-muc/"]').first()
