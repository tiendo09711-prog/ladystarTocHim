...
class CheckoutService
{
    ...
    private function authenticatedSummary(User $user, ?string $couponCode): array
    {
        ...
        if (! $cart || $cart->items->isEmpty()) throw ValidationException::withMessages(['cart' => 'Giỏ hàng đang trống.']);
    ...
    private function guestSummary(array $items, ?string $couponCode): array
    {
        ...
        if ($variants->count() !== $variantIds->unique()->count()) throw ValidationException::withMessages(['items' => 'Giỏ hàng chứa biến thể không hợp lệ.']);
    ...
    private function createOrderItem(Order $order, Branch $branch, array $line, ?User $user): void
    {
        ...
        $this->inventoryService->reserve($inventory, $line['quantity'], $user?->id);
    ...
    {
        if (empty($payload['coupon_code']) || $summary['discount_amount'] <= 0) return;
    ...
    private function discount(?User $user, ?string $code, float $subtotal): float
    {
        ...
        if ($coupon->usage_limit_per_user) {
            if (! $user) throw ValidationException::withMessages(['coupon_code' => 'Mã giảm giá này chỉ áp dụng cho khách hàng đã đăng nhập.']);
            $used = DB::table('coupon_usages')->where('coupon_id', $coupon->id)->where('user_id', $user->id)->count();
            if ($used >= $coupon->usage_limit_per_user) throw ValidationException::withMessages(['coupon_code' => 'Bạn đã sử dụng hết lượt của mã giảm giá này.']);
        }
